...
bool mcp_session_validate_id(const char* session_id) {
  ...
  if (len != 36)
    return false;  // UUID4格式：xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx

  // 检查UUID格式：8-4-4-4-12
  if (session_id[8] != '-' || session_id[13] != '-' || session_id[18] != '-' ||
      session_id[23] != '-') {
    return false;
  }
...
static void* session_cleanup_thread(void* arg) {
  ...
  while (manager->cleanup_running) {
    uint32_t remaining_ms = (uint32_t)(manager->config.cleanup_interval * 1000);
    while (manager->cleanup_running && remaining_ms > 0) {
      uint32_t sleep_ms = remaining_ms > 100 ? 100 : remaining_ms;
      if (hal != NULL && hal->thread.sleep_ms != NULL) {
        hal->thread.sleep_ms(sleep_ms);
      } else {
...
int mcp_session_manager_start(mcp_session_manager_t* manager) {
  ...
  if (manager->config.auto_cleanup) {
    ...
    if (thread_result == 0) {
      manager->cleanup_thread = thread_handle;
    } else {
      manager->cleanup_running = false;
      pthread_mutex_unlock(&manager->manager_mutex);
...
int mcp_session_manager_stop(mcp_session_manager_t* manager) {
  ...
  // 等待清理线程结束
  const mcp_platform_hal_t* hal = mcp_platform_get_hal();
  if (hal == NULL || hal->thread.join == NULL || hal->thread.join(manager->cleanup_thread) != 0) {
    mcp_log_warn("Failed to join session cleanup thread");
  }
  manager->cleanup_thread = NULL;
...
mcp_session_t* mcp_session_manager_create_session(mcp_session_manager_t* manager,
  ...
  for (size_t i = 0; i < manager->session_capacity; i++) {
    if (manager->sessions[i] && strcmp(manager->sessions[i]->session_id, id) == 0) {
      pthread_rwlock_unlock(&manager->sessions_lock);
      hal->memory.free(id);
...
// 移除会话
int mcp_session_manager_remove_session(mcp_session_manager_t* manager, const char* session_id) {
  if (!manager || !session_id)
    return -1;
...
const char* mcp_session_state_to_string(mcp_session_state_t state) {
  switch (state) {
    case MCP_SESSION_STATE_CREATED:
      return "CREATED";
    case MCP_SESSION_STATE_INITIALIZING:
      return "INITIALIZING";
    case MCP_SESSION_STATE_ACTIVE:
      return "ACTIVE";
    case MCP_SESSION_STATE_INACTIVE:
      return "INACTIVE";
    case MCP_SESSION_STATE_EXPIRED:
      return "EXPIRED";
    case MCP_SESSION_STATE_TERMINATED:
      return "TERMINATED";
    default:
      return "UNKNOWN";
  }
