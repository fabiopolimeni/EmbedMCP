...
static bool mcp_transport_test_fail_next_session_id_duplication = false;
...
mcp_transport_t*
mcp_transport_create_http_with_path(int port, const char* bind_address, const char* endpoint_path) {
  mcp_transport_t* transport = mcp_transport_create(MCP_TRANSPORT_HTTP);
  if (!transport)
    return NULL;
  ...
  config->config.http.endpoint_path =
    endpoint_path ? hal_strdup(hal, endpoint_path) : hal_strdup(hal, "/mcp");
...
int mcp_transport_init(mcp_transport_t* transport, const mcp_transport_config_t* config) {
  if (!transport || !transport->interface || !transport->interface->init)
    return -1;
...
int mcp_transport_start(mcp_transport_t* transport) {
  if (!transport || !transport->interface || !transport->interface->start)
    return -1;

  if (transport->state != MCP_TRANSPORT_STATE_STOPPED)
    return -1;
...
int mcp_transport_stop(mcp_transport_t* transport) {
  if (!transport || !transport->interface || !transport->interface->stop)
    return -1;
...
int mcp_connection_send(mcp_connection_t* connection, const char* message, size_t length) {
  if (!connection || !connection->transport || !connection->transport->interface)
    return -1;

  if (!connection->transport->interface->send)
    return -1;
...
int mcp_connection_close(mcp_connection_t* connection) {
  ...
  if (!connection->transport->interface->close_connection)
    return -1;
...
int mcp_connection_set_session_id(mcp_connection_t* connection, const char* session_id) {
  ...
  char* duplicated_session_id = NULL;
  if (mcp_transport_test_fail_next_session_id_duplication) {
    mcp_transport_test_fail_next_session_id_duplication = false;
  } else {
    duplicated_session_id = hal_strdup(hal, session_id);
  }
  ...
  hal_free(hal, connection->session_id);
  connection->session_id = duplicated_session_id;

  return 0;
}

void mcp_transport_debug_fail_next_session_id_duplication_for_test(void) {
  mcp_transport_test_fail_next_session_id_duplication = true;
}
...
mcp_transport_config_t* mcp_transport_config_create_http(int port, const char* bind_address) {
  ...
  config->config.http.bind_address =
    bind_address ? hal_strdup(hal, bind_address) : hal_strdup(hal, "0.0.0.0");
...
const char* mcp_transport_state_to_string(mcp_transport_state_t state) {
  switch (state) {
    case MCP_TRANSPORT_STATE_STOPPED:
      return "STOPPED";
    case MCP_TRANSPORT_STATE_STARTING:
      return "STARTING";
    case MCP_TRANSPORT_STATE_RUNNING:
      return "RUNNING";
    case MCP_TRANSPORT_STATE_STOPPING:
      return "STOPPING";
