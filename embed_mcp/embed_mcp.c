...
static int param_try_get_double_array(mcp_param_accessor_t* self,
                                      const char* name,
                                      double** out,
                                      size_t* count) {
  if (!out || !count) {
...
static int param_try_get_string_array(mcp_param_accessor_t* self,
                                      const char* name,
                                      char*** out,
                                      size_t* count) {
  if (!out || !count) {
...
static int param_try_get_int_array(mcp_param_accessor_t* self,
                                   const char* name,
                                   int64_t** out,
                                   size_t* count) {
  if (!out || !count) {
...
static void update_dynamic_capabilities(embed_mcp_server_t* server) {
  if (!server || !server->protocol || !server->protocol->config ||
      !server->protocol->config->capabilities) {
    return;
  }
...
static void on_transport_error(mcp_transport_t* transport,
                               int error_code,
...
int embed_mcp_run(embed_mcp_server_t* server, embed_mcp_transport_t transport) {
  ...
  if (transport == EMBED_MCP_TRANSPORT_STDIO) {
    ...
    server->transport =
      mcp_transport_create_http_with_path(server->port, server->host, server->path);
  }
  ...
  if (server->debug) {
    ...
    if (transport == EMBED_MCP_TRANSPORT_HTTP) {
      ...
      mcp_log_info("%s Server '%s' v%s started", transport_name, server->name, server->version);
    }
  }
...
int embed_mcp_quick_start(const char* name,
                          const char* version,
                          embed_mcp_transport_t transport,
                          int port) {
  embed_mcp_config_t config = {.name = name,
                               .version = version,
                               .host = "0.0.0.0",
                               .port = port,
                               .path = "/mcp",
...
typedef enum { TOOL_PARAM_MODE_TRADITIONAL, TOOL_PARAM_MODE_ADVANCED } tool_param_mode_t;
...
static cJSON* convert_universal_result_to_json(void* result, mcp_return_type_t return_type) {
  ...
  switch (return_type) {
    ...
    case MCP_RETURN_IMAGE:
      if (result) {
        mcp_image_result_t* img = (mcp_image_result_t*)result;
        cJSON* mcp_image = mcp_tool_create_image_result(img->data, img->mime_type);
        if (img->data)
          free(img->data);
        if (img->mime_type)
          free(img->mime_type);
      ...
      result_data = cJSON_CreateString("No image data");
      break;
    default:
      if (result) {
        free(result);
...
static cJSON* universal_function_wrapper(const cJSON* args, void* user_data) {
  ...
  if (!data || !data->wrapper_func) {
    return mcp_tool_create_error_result(
      MCP_TOOL_ERROR_INTERNAL, "Universal handler is not initialized", NULL);
  }

  // Create parameter accessor
  param_accessor_data_t accessor_data = {.args = args};
  mcp_param_accessor_t accessor = {.get_int = param_get_int,
...
static cJSON* schema_handler_wrapper(const cJSON* args, void* user_data) {
  ...
  if (!data || !data->handler) {
    return mcp_tool_create_error_result(
      MCP_TOOL_ERROR_INTERNAL, "Schema handler is not initialized", NULL);
  }
...
static int register_tool_internal(embed_mcp_server_t* server,
  ...
  mcp_tool_t* tool = mcp_tool_create_full(
    name, name, description, input_schema, NULL, execute_func, NULL, cleanup_func, handler_data);
  if (!tool) {
  ...
  if (strategy->mode == TOOL_PARAM_MODE_TRADITIONAL) {
    if (!param_descriptions || !param_types) {
      return fail_with_error(
        "Invalid parameters: param_descriptions and param_types are required in traditional mode");
    }
...
int embed_mcp_add_text_resource(embed_mcp_server_t* server,
  ...
  int result = mcp_resource_registry_add_text(
    server->resource_registry, uri, name, description, mime_type, content);
  if (result != 0) {
...
int embed_mcp_add_binary_resource(embed_mcp_server_t* server,
  ...
  int result = mcp_resource_registry_add_binary(
    server->resource_registry, uri, name, description, mime_type, data, size);
  if (result != 0) {
...
int embed_mcp_add_file_resource(embed_mcp_server_t* server,
  ...
  int result = mcp_resource_registry_add_file(
    server->resource_registry, uri, name, description, mime_type, file_path);
  if (result != 0) {
...
int embed_mcp_add_tool(embed_mcp_server_t* server,
  ...
  if (resolve_tool_param_strategy(
        param_names, param_descriptions, param_types, param_count, &strategy) != 0) {
    return -1;
  }
