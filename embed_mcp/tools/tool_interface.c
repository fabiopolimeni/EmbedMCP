...
static void set_validation_errorf(const char* fmt, const char* detail) {
  ...
  snprintf(
    g_validation_error_message, sizeof(g_validation_error_message), fmt, detail ? detail : "");
}
...
mcp_tool_t* mcp_tool_create(const char* name,
                            ...
                            void* user_data) {
  return mcp_tool_create_full(
    name, title, description, input_schema, NULL, execute_func, NULL, NULL, user_data);
}
...
void mcp_tool_destroy(mcp_tool_t* tool) {
  ...
  if (tool->output_schema)
    cJSON_Delete(tool->output_schema);
...
cJSON* mcp_tool_execute(const mcp_tool_t* tool, const cJSON* parameters) {
  if (!tool || !tool->execute) {
    return mcp_tool_create_error_result(
      MCP_TOOL_ERROR_INTERNAL, "Tool or execute function is null", NULL);
  }
  ...
  if (tool->input_schema &&
      !mcp_tool_validate_parameter_against_schema(parameters, tool->input_schema)) {
    char* error_msg = mcp_tool_get_validation_error_message(parameters, tool->input_schema);
    cJSON* result =
      mcp_tool_create_validation_error(error_msg ? error_msg : "Schema validation failed");
    free(error_msg);
    return result;
  }
...
bool mcp_tool_validate(const mcp_tool_t* tool) {
  ...
  // Validate name
  if (!mcp_tool_validate_name(tool->name))
    return false;

  // Validate schemas if present
  if (tool->input_schema && !mcp_tool_validate_schema(tool->input_schema))
    return false;
  if (tool->output_schema && !mcp_tool_validate_schema(tool->output_schema))
    return false;
...
bool mcp_tool_validate_schema(const cJSON* schema) {
  if (!schema)
    return true;  // NULL schema is valid (no validation)
...
cJSON* mcp_tool_create_object_schema(const char* description,
...
char* mcp_tool_get_validation_error_message(const cJSON* value, const cJSON* schema) {
  ...
  if (!schema)
    return strdup("No schema provided");
  if (!value)
    return strdup("No value provided");
...
cJSON* mcp_tool_create_memory_error(void) {
  return mcp_tool_create_error_result(
    MCP_TOOL_ERROR_MEMORY, "Tool execution exceeded memory limit", NULL);
}
...
cJSON* mcp_tool_create_image_result(const char* base64_data, const char* mime_type) {
  ...
  cJSON_AddStringToObject(image_block, "data", base64_data ? base64_data : "");
  cJSON_AddStringToObject(image_block, "mimeType", mime_type ? mime_type : "image/png");
